...
class Kernel extends BaseKernel
{
    ...
    private static function loadEnv(Dotenv $dotenv, $path)
    {
        ...
        if (isset($usersRequestUrl) && isset($usersRealm)) {
            $dotenv->populate(
                [
                    'OAUTH_KEYCLOAK_REQUEST_URL' => "$usersRequestUrl/admin/realms/$usersRealm/",
                    'OAUTH_KEYCLOAK_LOGOUT' => "$usersRequestUrl/realms/$usersRealm/protocol/openid-connect/logout?redirect_uri=",
                    'KEYCLOAK_REFRESH_TOKEN_URL' => "$usersRequestUrl/realms/$usersRealm/",
                    'KEYCLOAK_REFRESH_TOKEN_URL_ENDPOINT' => "$usersRequestUrl/realms/$usersRealm/protocol/openid-connect/token",
                ]
            );
        }

        $devicesRequestUrl = getenv('DEVICES_KEYCLOAK_AUTH_URL');
        $devicesRealm = getenv('DEVICES_KEYCLOAK_REALM');
        if (isset($devicesRequestUrl) && isset($devicesRealm)) {
            $dotenv->populate(
                [
                    'DEVICES_KEYCLOAK_REQUEST_URL' => "$devicesRequestUrl/admin/realms/$devicesRealm/",
                    'DEVICES_KEYCLOAK_TOKEN_URL' => "$devicesRequestUrl/realms/$devicesRealm/protocol/openid-connect/token",
                ]
            );
        }
    }
